...
class OwnerProfileSetupScreen extends StatefulWidget {
  ...
  const OwnerProfileSetupScreen({super.key, required this.profile});

  @override
  State<OwnerProfileSetupScreen> createState() {
...
class _OwnerProfileSetupScreenState extends State<OwnerProfileSetupScreen> {
  ...
  void initState() {
    ...
    _controller = EditProfileController(initialProfile: widget.profile);
  ...
  double _progress(EditProfileState state) {
    var completed = 0;

    if (_controller.nameController.text.trim().isNotEmpty) {
      completed++;
    }

    if (_controller.phoneController.text.trim().isNotEmpty) {
      completed++;
    }

    if (_controller.addressController.text.trim().isNotEmpty) {
      completed++;
    }
  ...
  Future<void> _selectPhoto() async {
        ...
        return SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text('Fotografía de perfil', style: DogGoTheme.title(size: 21)),
    ...
    if (!selected && _controller.state.error != null) {
  ...
  Future<void> _save() async {
    ...
    if (_controller.addressController.text.trim().isEmpty) {
      _showMessage('Agrega la dirección de recogida.', error: true);
      return;
    }

    if (!_controller.state.hasLocation) {
      _showMessage('Selecciona el punto de recogida en el mapa.', error: true);
      return;
    }

    if (_controller.state.selectedStateCode == null ||
        _controller.state.selectedMunicipalityCode == null) {
      _showMessage('Selecciona tu estado y municipio.', error: true);
      return;
    }
    ...
    _showMessage('Tu perfil de dueño está listo.');
  ...
  void _showMessage(String message, {bool error = false}) {
    ...
    messenger
      ...
      ..showSnackBar(
        SnackBar(
          backgroundColor: error ? DogGoTheme.red : DogGoTheme.teal,
          content: Row(
            children: [
              Icon(
                error
  ...
  Widget _buildBody(EditProfileState state) {
    if (state.loading) {
      return const DogGoLoadingView(message: 'Preparando tu perfil...');
    }

    if (state.error != null && !state.ownerProfileLoaded) {
      return Center(
  ...
  Widget _buildWelcomeCard(EditProfileState state) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [DogGoTheme.tealDark, DogGoTheme.teal],
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: const AlwaysStoppedAnimation(DogGoTheme.orange),
            ),
          ),
        ],
      ),
  ...
  Widget _buildPickupSection(EditProfileState state) {
    return _SetupSection(
      number: '1',
      icon: Icons.location_on_outlined,
      title: 'Punto de recogida',
      subtitle: 'Será la ubicación predeterminada para solicitar paseos.',
      ...
      child: Column(
        children: [
          ...
          DogGoMapPreview(
            ...
            height: 205,
            emptyText: 'Fija tu punto de recogida',
  ...
  Widget _buildReferencesSection(EditProfileState state) {
    return _SetupSection(
      number: '2',
      icon: Icons.signpost_outlined,
      title: 'Ayuda al paseador',
      subtitle: 'Agrega referencias para localizar el domicilio fácilmente.',
      child: Column(
        children: [
          _SetupField(
            controller: _controller.referencesController,
            label: 'Referencias',
            hint: 'Color de casa, entrecalles, portón...',
            icon: Icons.signpost_outlined,
            maxLines: 3,
            textInputAction: TextInputAction.next,
            validator: (value) {
  ...
  Widget _buildPersonalSection(EditProfileState state) {
    return _SetupSection(
      number: '3',
      icon: Icons.person_outline_rounded,
      title: 'Tus datos',
      subtitle: 'Información para coordinar el servicio contigo.',
      child: Column(
        children: [
          Row(
            children: [
  ...
  Widget _buildPreferencesSection() {
    return _SetupSection(
      ...
      title: 'Preferencias',
      subtitle: 'Opcional: comparte información útil para futuros paseos.',
      child: Column(
        children: [
          _SetupField(
            controller: _controller.descriptionController,
            label: 'Sobre ti y tus mascotas',
            hint: 'Cuéntanos algo importante',
          ...
          _SetupField(
            ...
            hint: 'Horarios, cuidados o indicaciones',
  ...
  Widget _buildBottomAction(EditProfileState state) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: DogGoTheme.softShadow(offset: const Offset(0, -5), blur: 18),
      ),
      child: ElevatedButton.icon(
        ...
        label: Text(
          state.saving ? 'Guardando perfil...' : 'Guardar y comenzar',
        ),
      ),
    );
  }
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: requiredSection
              ? DogGoTheme.teal.withValues(alpha: .25)
              : DogGoTheme.border,
        ),
            ...
            children: [
              Container(
                ...
                child: Stack(
                  ...
                  children: [
                    Icon(icon, color: DogGoTheme.teal, size: 22),
                    Positioned(
                      right: 3,
                      top: 3,
                      child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Flexible(
                          child: Text(title, style: DogGoTheme.title(size: 17)),
                        ),
                        if (requiredSection)
                          Container(
...
class _CompactProfilePhoto extends StatelessWidget {
...
class _PhotoPlaceholder extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Icon(Icons.person_rounded, color: DogGoTheme.teal, size: 36);
  }
}
