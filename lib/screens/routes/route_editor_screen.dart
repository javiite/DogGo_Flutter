...
enum _EditorTool { route, checkpoint }

class RouteEditorScreen extends StatefulWidget {
  final LatLng? initialCenter;
  final DoggoRouteDraft? initialDraft;

  const RouteEditorScreen({super.key, this.initialCenter, this.initialDraft});
...
class _RouteEditorScreenState extends State<RouteEditorScreen> {
  ...
  void initState() {
    ...
    if (draft != null) {
      ...
      _allowedRadiusMeters = draft.allowedRadiusMeters;
  ...
  List<DoggoRoutePoint> get _pathPoints {
    return _points.where((point) => !point.isCheckpoint).toList(growable: false)
  ...
  void _addPoint(LatLng position) {
    final nextOrder = _points.isEmpty
        ? 0
        : _points.map((point) => point.order).reduce((a, b) => a > b ? a : b) +
              1;

    if (_tool == _EditorTool.checkpoint) {
      final checkpointNumber = _checkpoints.length + 1;
      ...
      setState(() {
        _points.add(
          DoggoRoutePoint(
            ...
            type: 'Checkpoint',
            name: 'Punto de aviso $checkpointNumber',
      ...
      _showMessage('Punto de aviso agregado.');
  ...
  void _undo() {
    if (_points.isEmpty) {
      _showMessage('Todavía no has agregado puntos.');
      return;
    }
  ...
  Future<void> _editCheckpoint(DoggoRoutePoint checkpoint) async {
    final nameController = TextEditingController(text: checkpoint.name ?? '');

    var radius = checkpoint.alertRadiusMeters ?? 50;

    var notify = checkpoint.notifyOnArrival;
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Punto de aviso', style: DogGoTheme.title(size: 22)),
                    ...
                    Row(
                      children: [
                        ...
                        Text('$radius m', style: DogGoTheme.label()),
                      ],
                    ),
                    Slider(
                    ...
                    SwitchListTile(
                      ...
                      subtitle: const Text('Avisar al dueño al llegar.'),
                      value: notify,
                      onChanged: (value) {
                        setSheetState(() {
                          notify = value;
                    ...
                    FilledButton.icon(
                      onPressed: () {
                        final index = _points.indexWhere(
                          (point) => point.order == checkpoint.order,
                        );

                        if (index >= 0) {
                          setState(() {
                            _points[index] = checkpoint.copyWith(
  ...
  void _finish() {
    ...
    if (name.isEmpty) {
      _showMessage('Escribe un nombre para la ruta.');
      return;
    }

    final minimumPoints = _isArea ? 3 : 2;

    if (_pathPoints.length < minimumPoints) {
      _showMessage(
        _isArea
    ...
    final normalizedPoints = <DoggoRoutePoint>[];

    final ordered = [..._points]..sort((a, b) => a.order.compareTo(b.order));

    for (var index = 0; index < ordered.length; index++) {
      normalizedPoints.add(ordered[index].copyWith(order: index));
    }
  ...
  String _errorMessage(Object error) {
    ...
    return message.isEmpty ? 'No se pudo obtener tu ubicación.' : message;
  }
          ...
          Positioned.fill(
            child: FlutterMap(
              ...
              children: [
                ...
                if (_isArea && pathPositions.length >= 3)
                  PolygonLayer(
                    polygons: [
                      Polygon(
                ...
                if (!_isArea && pathPositions.length >= 2)
                  PolylineLayer(
                    polylines: [
                      Polyline(
                        points: pathPositions,
                    ...
                    circles: _checkpoints
                        .map(
                          (checkpoint) => CircleMarker(
                            ...
                            color: DogGoTheme.orange.withValues(alpha: .15),
                            borderColor: DogGoTheme.orange,
                            borderStrokeWidth: 2,
                          ),
                        )
                        .toList(),
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    Text(
                      'Tipo de control',
                      style: DogGoTheme.label(color: DogGoTheme.muted),
                    ),
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _ChoiceButton(
                            selected: _tool == _EditorTool.route,
                            icon: Icons.add_location_alt_outlined,
                            label: _isArea ? 'Dibujar límite' : 'Trazar ruta',
                            onTap: () {
                              setState(() {
                                _tool = _EditorTool.route;
                              });
                            },
                          ),
                        ),
                        ...
                        Expanded(
                          child: _ChoiceButton(
                            selected: _tool == _EditorTool.checkpoint,
                            icon: Icons.flag_outlined,
                            label: 'Punto de aviso',
                            onTap: () {
                              setState(() {
                                _tool = _EditorTool.checkpoint;
                              });
                            },
                          ),
                        ),
                    ...
                    Slider(
                      value: _allowedRadiusMeters.toDouble(),
                      min: 20,
                      max: 1000,
                      divisions: 49,
                      label: '$_allowedRadiusMeters m',
                      onChanged: (value) {
                        setState(() {
                          _allowedRadiusMeters = value.round();
                        });
                      },
                    ),
                    ...
                    FilledButton.icon(
                      onPressed: _finish,
                      icon: const Icon(Icons.check_rounded),
                      label: const Text('Usar este recorrido'),
                    ),
                  ],
                ),
              );
...
class _InstructionCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final checkpointTool = tool == _EditorTool.checkpoint;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: DogGoTheme.softShadow(opacity: .09),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              checkpointTool
                  ...
                  : Icons.route_rounded,
              color: checkpointTool ? DogGoTheme.orange : DogGoTheme.teal,
            ),
          ),
...
class _ChoiceButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: selected ? DogGoTheme.tealLight : DogGoTheme.cream,
...
class _RoutePointMarker extends StatelessWidget {
  ...
  const _RoutePointMarker({required this.number, required this.isArea});

  @override
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: isArea ? DogGoTheme.purple : DogGoTheme.teal,
        shape: BoxShape.circle,
        border: Border.all(color: Colors.white, width: 3),
        boxShadow: DogGoTheme.softShadow(),
      ),
