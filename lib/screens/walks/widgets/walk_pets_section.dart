...
class WalkPetsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pets = walk.requestedPets.isNotEmpty
        ? walk.requestedPets
        : walk.pets;
        ...
        children: [
         ...
         if (state.canEditRequestedPets) ...[
          ...
          OutlinedButton.icon(
            ...
            label: const Text(
            'Editar mascotas de la solicitud',
            ),
...
Future<void> _openOwnerPetEditor(
  ...
  final result =
      await controller.updateRequestedPets(
    petIds: selectedIds,
  );
  ...
  Future<void> _openProposal(
    ...
    final initialIds = walk.requestedPets
        .map((pet) => pet.id)
        .toSet();

    final selectedIds =
        Set<int>.from(initialIds);
    ...
    if (!context.mounted ||
        submittedPetIds == null) {
      return;
    }

    final result =
        await controller.proposePetChange(
  ...
  Future<void> _respond(
                ...
                TextField(
                  ...
                  decoration:
                    ...
                    labelText:
                        'Motivo opcional',
                    hintText:
                        'Explica por qu̩ no aceptas el cambio.',
                  ),
                ),
              ],
            ],
    ...
    final result =
        await controller.respondPetChange(
      accept: accept,
