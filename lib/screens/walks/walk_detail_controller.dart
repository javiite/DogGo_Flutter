...
class WalkDetailController extends ChangeNotifier {
    ...
    try {
      ...
      _setState(
        _state.copyWith(
          ...
          role: results[1]?.toString().trim().isNotEmpty ==
                  true
              ? results[1].toString()
              : _state.role,
        ),
      ...
      // El detalle puede seguir funcionando con la
      // información recibida desde la pantalla anterior.
    }
  ...
  Future<void> loadDetail() async {
    ...
    try {
      final response =
          await PaseosService.obtenerPaseoPorId(id);
      ...
      final detailMap =
          _normalizeDetail(response['data']);
    ...
    try {
      ...
      switch (action) {
        case WalkDetailAction.accept:
          response =
              await PaseosService.aceptarPaseo(id);
          successMessage =
        ...
        case WalkDetailAction.reject:
          response =
              await PaseosService.rechazarPaseo(id);
          successMessage =
        ...
        case WalkDetailAction.start:
          response =
              await PaseosService.iniciarPaseo(id);
          successMessage =
        ...
        case WalkDetailAction.finish:
          response =
              await PaseosService.finalizarPaseo(id);
          successMessage =
      ...
      if (action == WalkDetailAction.finish ||
        ...
        try {
          ...
          // La acción del paseo ya fue completada;
          // no debe fallar por el servicio local.
        }
      }
